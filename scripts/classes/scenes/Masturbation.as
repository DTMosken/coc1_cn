package classes.scenes
{
   ...
   public class Masturbation extends BaseContent
   {
      ...
      public function setMasturbateButton(param1:Boolean = false, param2:int = 8) : void
      {
         ...
         {
            addButton(param2,"自慰",param1 ? masturbateGo : masturbateMenu).hint("释放你积累的性欲。").disableIf(get_player().get_lust() < 30,"你的欲望度尚不足够。");
         }
      }
      ...
      public function fappingItems(param1:Boolean = true) : Boolean
      {
         ...
         if(get_player().hasKeyItem("All-Natural Onahole") && _loc2_)
         {
            if(param1)
            {
               addNextButton("纯天然名器",allNaturalOnaholeUse).hint("一个纯天然名器，这件器具看起来更像是一个球茎状生物，而不是性玩具。尽管如此，它露出的滑腻孔口看上去依然十分诱人。","纯天然名器");
            }
            _loc3_ = true;
         }
         if(get_player().hasKeyItem("Deluxe Onahole") && _loc2_)
         {
            if(param1)
            {
               addNextButton("豪华名器",deluxeOnaholeUse).hint("这是一件豪华名器，采用上等材料和精湛工艺制成，旨在将使用者送上欢愉的巅峰。","豪华名器");
            }
            _loc3_ = true;
         }
         if(get_player().hasKeyItem("Plain Onahole") && _loc2_)
         {
            if(param1)
            {
               addNextButton("名器",onaholeUse).hint("这就是所谓的“名器”。这件器具是一个带有纹理的简易套筒，设计成能舒适地套住男性器官，带来快感。","名器");
            }
            _loc3_ = true;
         }
      ...
      public function centaurDudesGetHorseAids() : void
      {
         ...
         if(get_player().keyItemv1("Fake Mare") == 0)
         {
            if(get_player().cor < 50)
            {
               outputText("你决定试试惠特尼给你的那具母马般的阴茎套。你花了几分钟，笨拙地把那坨金属拖到某个僻静处并架设起来。完事后，你站在一具木铁制成的母马复制品身后，它的高度已经调到最适合你的位置。你打量着[say:她]，目光被她两腿之间那对光滑的黑色名器唇瓣吸引过去；它被巧妙地做成了母马小穴的形状，而上方似乎还有一个更小的开口，模拟着肛门。[pg]");
            }
            else
            {
               outputText("你决定玩玩惠特尼给你的母马形阴茎套。你把它从储物处拖出来，花了几分钟在营地中央架设好。完事后，你站在一具木铁制成的母马复制品身后，它的高度已经调到最适合你的位置。你打量着[say:她]，目光被她两腿之间那对光滑的黑色名器唇瓣吸引过去；它被巧妙地做成了母马小穴的形状，而上方似乎还有一个更小的开口，模拟着肛门。[pg]");
            }
         }
