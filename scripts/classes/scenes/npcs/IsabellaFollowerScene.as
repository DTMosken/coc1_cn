   ...
   public class IsabellaFollowerScene extends NPCAwareContent implements SelfDebug, SelfSaving
   {
      ...
      public function toggleIsabellasMorningWoodChopping() : void
      {
         ...
         else
         {
            ...
            if(get_player().shortestCockLength() > get_isabellaScene().tinyPenis())
            {
               outputText("我[if (isabellaAccent) {费|会}]的，但你得先把[if (isabellaAccent) {内|那}]个怪物变小。我没法处理[if (isabellaAccent) {则样的|这样的}]东西！");
            }
            else
            {
               outputText("当然；[if (isabellaAccent) {唔|我}]们可不[if (isabellaAccent) {箱|想}]你欲求不满地在[if (isabellaAccent) {坏|外}]面和恶魔战斗。谁知道[if (isabellaAccent) {费|会}]发生什么？");
            }
            outputText("[sayend]");
      ...
      public function toggleIsabellaContraceptives() : void
      {
         ...
         if(FlagDict_Impl_.arrayReadInt(KFLAGS.flags,2212) == 1)
         {
            ...
            outputText("[pg]牛娘通情达理地点了点头。[saystart]呀，[if (isabellaAccent) {缺|确}]实有道理。我们在这里过着艰苦的生活；这个时代[if (isabellaAccent) {，他们}]太危险了，很难指望你照顾一个怀孕的[if (isabellaAccent) {羽人|女人}]，更别说小宝宝了，[if (isabellaAccent) {系吧|是吧}]？");
            if(get_isabellaScene().totalIsabellaChildren() > 0)
            {
               outputText("此外，[if (isabellaAccent) {唔|我}]们已经有 " + Utils.num2Text(get_isabellaScene().totalIsabellaChildren()) + " 个小" + (get_isabellaScene().totalIsabellaChildren() == 1 ? "婴儿" : "婴儿") + "需要照顾了。");
            }
            outputText("[sayend]");
         ...
         else
         {
            ...
            if(get_isabellaScene().totalIsabellaChildren() > 0)
            {
               outputText("[pg]她调皮地对你咧嘴一笑。[say: " + Utils.Num2Text(get_isabellaScene().totalIsabellaChildren()) + " 个小" + (get_isabellaScene().totalIsabellaChildren() == 1 ? "婴儿" : "婴儿") + "[if (isabellaAccent) {怀|还}]不够，嗯？你[if (isabellaAccent) {还想要|想要}]更多？]");
            }
      ...
      public function repeatGermanBratwurstInCamp() : void
      {
         ...
         outputText("这位自信的女人继续着她的旋转，非常清楚她对你产生的影响。[say: 不，还没到时候，]她命令道，她那滴着汗水的屁股危险地靠近[onecock]。伊莎贝拉将她迷人的背部抬得更高，把它带到你的面前。她慢慢地扭动着，然后张开双腿，让你看到她闪闪发光的阴道和紧紧闭合的菊花从她暗色的脸颊后露出来。[say:[if (isabellaAccent) {号湿，|好湿，}]]她呻吟着，然后恢复了她的摇晃，");
         outputText("[say: 好热……我让你觉得热了吗，[name]？你的肉棒已经为我抽动了吗？是不是又酸又漏，弄得自己一团糟[if (isabellaAccent) {，呀哈}]？][pg]");
         ...
         outputText("[say: 我说了还没到时候呢。你还[if (isabellaAccent) {箱不箱|想不想}]我帮你高潮了？]");
         outputText("你对她点点头，在你的欲望被你渴望的对象打断后，你感觉稍微恢复了一些控制。伊莎贝拉半闭着眼睛回头看着你，低语道，");
         outputText("[say: 乖[boy]。放松，伊莎贝拉妈妈[if (isabellaAccent) {费|会}]照顾你的。][pg]");
         ...
         if(get_player().cockArea(_loc1_) <= 38)
         {
            ...
            outputText("伊莎贝拉粗暴地抬起身体，发出啧啧声，[say: 不，[if (isabellaAccent) {则可补|这可不}]行。]");
         ...
         else
         {
            ...
            outputText("。她用悦耳的声音戏弄你，带着一丝轻蔑的意味，[say: 这么快就要射了？[if (isabellaAccent) {浑|很}]好，那就[if (isabellaAccent) {及续|继续}]吧。让你那恶心、淫荡的鸡巴控制你吧。我能感觉到它在[if (isabellaAccent) {唔的|我的}]屁股上滴滴答答。照你那肥大、粗壮的鸡巴的吩咐去做，射在伊莎贝拉妈妈的屁股上吧。又大又粗的鸡巴似乎总是射[if (isabellaAccent) {滴|得}]太快了。][pg]");
      ...
      public function receiveAllTheCowTOngues() : void
      {
         ...
         outputText("[say: 该[if (isabellaAccent) {向|上}]主菜了，]");
      ...
      public function milktasticLacticLactation() : void
      {
         ...
         spriteSelect(SpriteDb.get_s_isabella());
         clearOutput();
         outputText("[say:[if (isabellaAccent) {哦，唔的奶子……|哦，我的奶子……}]][pg]");
      ...
      public function izzyMilkingMeinMilkersMya2() : void
      {
         ...
         outputText("[if (isabellaAccent) {系|是}]的，我——哦——有太多的事情要做，以至于我没有时间排空哦-哦-出我-我-我的……我的……哞哞大奶子！啊，啊，啊！[sayend] 伊莎贝拉抽搐着，弓起背，她裙子前面的黑斑明显变大了；她刚刚因为排空了大量的乳汁而高潮了！惠特尼注意到你在盯着看，苦笑了一下，拍了拍你的肩膀。[say: 以前也经常发生这种事，] 她低声说。[say: 她会在一周后进来抱怨她的奶子有多痛，把机器弄得全是奶，自己也爽得喷水。我提议让她住在农场里，这样她就不用等那么久了，但她听不进去。我想她太独立了；就像你一样。] 惠特尼把手拿开，走到装瓶站，把它打开。[pg]");
         outputText("随着奶流减弱，背带将面色潮红、气喘吁吁的伊莎贝拉放到了地上。你走过去查看她的情况，她虚弱地伸出双臂抱住你，用她那依然赤裸但已得到释放的乳房蹭着你，并给了你一个湿漉漉的吻。[say: 哦，[name]，太感[if (isabellaAccent) {系|谢}]你提醒我[if (isabellaAccent) {介|这}]个地方了。][pg]");
      ...
      public function izzyMilkingMeinMilkersMya() : void
      {
         ...
         outputText("虽然一想到能喝到她那起泡的奶油般香甜的乳汁你就垂涎欲滴，但你完全怀疑自己是否有能力在不引起糖休克的情况下处理掉这么多奶水，于是你把这告诉了她。她垂头丧气地开始抽泣，大声哭诉道：[say:[if (isabellaAccent) {内——吸溜——我|那——吸溜——我}]该拿我这些奶水怎么办啊？呜呜呜！][pg]");
         ...
         outputText("[say: 哦，[name]，我完全忘了[if (isabellaAccent) {内|那}]些挤奶器，都是[if (isabellaAccent) {医|因}]为营地里的[if (isabellaAccent) {内|那}]些[if (isabellaAccent) {伙|活}]儿！好，我们马上去[if (isabellaAccent) {内|那}]儿吧！][pg]");
      ...
      public function isabellaTalksAboutPotentialPregnancy() : void
      {
         ...
         outputText("[pg][say: 啊，[name]，我正[if (isabellaAccent) {喜|希}]望能见到你，]她回答道。她紧张地看着地面，用一只分趾的蹄子刨着干燥的泥土，突然脱口而出，[say: 你对孩子有[if (isabellaAccent) {啥|什}]么看法，[name]？]");
         outputText("[pg]你惊讶地看着她，问她为什么要问你这个。伊莎贝拉依然保持着紧张的神情，然后尴尬地开口道：[say: 我……[if (isabellaAccent) {恩|嗯}]，我一直以[if (isabellaAccent) {为|回}]，总有一天，我[if (isabellaAccent) {费|会}]有一个家庭，但首先我必须找到一个[if (isabellaAccent) {积德推服|值得托付}]的男人，[if (isabellaAccent) {兰|然}]后就[if (isabellaAccent) {系莫|是冒}]险，而且……[if (isabellaAccent) {恩|嗯}]，在来到[if (isabellaAccent) {介个系界|这个世界}]之前，我一直没有机会。但是，现在我有了你……我明白有很多理由不这么做，但是，如果你真的[if (isabellaAccent) {箱活|想和}]我组建一个家庭，我[if (isabellaAccent) {劝|愿}]意。好吗？我只是[if (isabellaAccent) {箱|想}]告诉你这些，希望你能记住[if (!isabellaAccent) {这事}]。]");
      ...
      public function isabellaTakesOviElixir() : void
      {
         ...
         {
            outputText("你递给伊莎贝拉一瓶促孕药剂，提到这会加快婴儿的发育。犹豫爬上了她的脸庞。[if (isabellaAccent) {[say: 介样好吗？伊扎贝拉不想让孩子变异……]|[say: 这样好吗？我不想让我们的孩子变异……]}]据你所知，这药剂是安全的，你向她保证了这一点。这位牛娘拿着瓶子静静地待了一会儿，然后接过来一口气喝光了。她本质上还是人类，所以你不确定你到底缩短了多少时间。");
         }
      ...
      public function isabellaMorningWakeupCall() : void
      {
         ...
         {
            outputText("美味！你为[if (isabellaAccent) {伊扎贝拉|伊莎贝拉}]妈妈射得这么猛，不是吗？[sayend]吃饱喝足的牛娘深情地捏了捏你的" + get_player().cockDescript(_loc2_) + "，然后喃喃自语道，[saystart]真好吃……");
         }
         else if(isabellaAccent())
         {
      ...
      public function isabellaBurps() : void
      {
         ...
         if(FlagDict_Impl_.arrayReadInt(KFLAGS.flags,383) == 0)
         {
            outputText("[say:[if (isabellaAccent) {亲爱的，内是啥么？|那是什么？}]]伊莎贝拉天真地问道，打量着你递给她的那瓶浑浊液体。你告诉她这是超级牛奶，基本上就是她变成现在这样时喝的药剂的强化版。她的眼神有些迷离，心不在焉地挤压着自己那对巨大的乳房，这是她发生变化的最明显标志。[say:你想让我多喝点？]她问道，醇厚的声音中只带着一丝不安。");
            if(isabellaAccent())
            {
            ...
            outputText("[pg]在你保证她除了明显的牛娘特征外不会有其他变化后，她明显平静下来，对你露出灿烂的笑容，伸手去拿药剂。[say:既然[if (isabellaAccent) {介|这}]样，那就干杯吧！]");
      ...
      public function getMilk() : void
      {
         ...
         if(FlagDict_Impl_.arrayReadInt(KFLAGS.flags,375) == -1)
         {
            outputText("伊莎贝拉点点头，从她最近在农场挤奶的备用品中拿了一瓶给你。[say: 给你！挤出来[if (isabellaAccent) {怀|还}]不到二十四小时，[if (isabellaAccent) {豪么?|好吗？}]][pg]");
         }
         else
         {
      ...
      public function getIzzyBurped() : void
      {
         ...
         {
            outputText("你完全无视了警告，立刻跑去帮助你的牛娘同伴解决她胃部不适的问题。她难以置信地看了你一眼，嘴里结结巴巴地抗议了十几次。然而，你心意已决，尽职尽责地揉着她的肚子，一心想减轻她的痛苦。[say:你……你……笨蛋！[if (isabellaAccent) {啥么-|什么-}]]");
            ...
            {
               outputText("[say:天哪，看看你！]伊莎贝拉惊呼道，挥手驱散了残留的雾气。[say:现在你肯定能和我[if (isabellaAccent) {一觉高下了，呀哈？|一较高下了，亲爱的！}]]");
      ...
      public function callForFollowerIsabella() : void
      {
         ...
         if(FlagDict_Impl_.arrayReadInt(KFLAGS.flags,1083) == 0)
         {
            ...
            outputText("她说道，[say:[if (isabellaAccent) {嗨，[name]！你些要伊莎贝拉做些啥么？|嗨，[name]！有什么我可以帮忙的吗？}]]");
            if(get_player().hasCock())
            {
               if(get_player().shortestCockLength() > get_isabellaScene().tinyPenis())
               {
         ...
         else
         {
            ...
            outputText("[pg][say:[if (isabellaAccent) {枣啊，[name]。伊莎贝拉能帮你什么忙吗？|你好，[name]。伊莎贝拉能帮你什么忙吗？}]]");
         }
         menu();
      ...
      public function acceptCowpology() : void
      {
         ...
         if(FlagDict_Impl_.arrayReadInt(KFLAGS.flags,383) == 1)
         {
            ...
            {
               outputText("伊莎贝拉决定采取主动，托起你的一只乳房，将它拉到嘴边。她那丰满的嘴唇包住你已经很敏感的乳头，这种感觉让你不由自主地后退。为了不让她占上风，你张大嘴巴，吞没了她那四个漏奶的乳头。你们俩就像在比赛一样，一边吸吮着一只乳房，一边揉捏着另一只。随着这场较量的继续，她的脸红得更深了，但直到你的大腿擦过她的大腿，你才真正意识到她的兴奋。大量的淫液涂抹在你的[skinfurscales]上。[pg]");
