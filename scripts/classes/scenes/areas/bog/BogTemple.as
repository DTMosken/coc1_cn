   ...
   public class BogTemple extends BaseContent implements SelfDebug, SelfSaving
   {
      ...
      public function templeMenu(param1:Boolean = true, param2:Boolean = true) : void
      {
         ...
         if(param1)
         {
            ...
            {
               outputText("[pg]" + (get_time().isTimeBetween(22.5,6.5) ? "玛丽埃尔占据着缺口的大半个帐篷里漆黑一片，寂静无声。你往里偷看，发现这个亡灵女孩蜷缩在睡袋里，周围堆满了成卷的布料和装有各种缝纫及露营用品的箱子。她睡着了。" : "亡灵裁缝玛丽埃尔占据了那个缺口，从外面拉起了一个大半个帐篷盖在上面，把它变成了她裁缝店的入口。女孩本人正坐在推到低矮残墙边的一张桌子旁，她周围的空间里散落着大量的缝纫工具、箱子，以及成卷的布料、线和丝带。她平常穿的衣服已经足够得体了，而一副旧夹鼻眼镜和一盏点燃的油灯则帮助她集中注意力。她" + (get_marielle().saveContent.commissionTime > get_time().get_totalTime() ? "没有注意到你，她的注意力完全集中在制作你要求的物品上。" : "似乎全神贯注于某事，你也不确定她是否注意到了你的进入。")));
            }
         }
         menu();
         addNextButton("玛丽埃尔",get_marielle().encounter).hint("靠近" + (get_time().isTimeBetween(22.5,6.5) ? "熟睡的" : "") + "裁缝。").disableIf(get_marielle().saveContent.commissionTime > get_time().get_totalTime() && get_time().isTimeBetween(6.5,22.5),"你或许应该让裁缝专心完成你的委托，不要去打扰她。").hideIf(get_marielle().saveContent.state != 1);
         _g = this;
      ...
      public function templeInspect(param1:int = -1) : void
      {
         ...
         else if(_loc2_ == 4)
         {
            ...
            {
               outputText("[pg]就在外面，玛丽埃尔建起了她的精品店，用她固定在神庙正面和周围植被上的半个帐篷有效地覆盖了整个洞。这位女裁缝本人正");
               ...
               else
               {
                  ...
                  outputText("[pg]她的帐篷里堆满了箱子、盒子和成卷的布料，它们被放在防护垫上，所以你无法确定外面的地面是否能提供更多线索。当你调查她桌子靠着的墙壁下半部分是否有明显的痕迹时——你什么也没发现——玛丽埃尔终于察觉到了你的靠近，她微微一惊，停下了手中的针线活。");
                  if(!(get_marielle().saveContent.insulted != 0 && get_marielle().saveContent.insulted != 2))
                  {
                     ...
                     outputText("[pg][say: 听起来很合理，不是吗？] 玛丽埃尔说道。[say: 只是，我从未见过如此]——她用一根手指紧紧缠绕着一缕金发——[say: 强大的生物……至少在这片区域没有。也许……] 她的声音渐渐变小，似乎不想说出她的想法，所以你问她是否有什么主意，以此让她继续说下去。她皱了皱眉。[say: 不。不，恐怕我不知道。这个地方有太多奇怪的野兽，还有，啊……强大而可怕的怪物。] 她松开头发，若有所思地扶了扶夹鼻眼镜，沉默在蔓延。");
      ...
      public function templeBathe() : void
      {
         ...
         if(get_marielle().marielleAvailable(false,false))
         {
            ...
            {
               outputText("[pg]你瞥了一眼玛丽埃尔，她正全神贯注地处理你的委托——以至于如果你就在这里安静地洗个澡，她可能都不会注意到。");
            }
            else
            {
               outputText("[pg]你瞥了一眼" + (get_time().isTimeBetween(22.5,6.5) ? "玛丽埃尔安静的帐篷，她的灯已经熄灭，女孩在她的货物和材料后面的某个地方睡着了。你怀疑如果你在这里洗澡会不会吵醒她——距离够远了，所以除非你决定弄出很大的动静，否则你不会打扰她睡觉。" : "玛丽埃尔，她显然正在一叠笔记上画着什么，铅笔在纸上快速、无声地飞舞。如果你在这里洗澡，她可能不会介意，甚至可能不会注意到。"));
            }
            menu();
            _g = this;
